    ...
    return (
        <>
            <PoiSlide imgSrc={info[currentIndex].bannerImage} imgAlt={info[currentIndex].alt} dotsLength={info.length} dotsIndex={currentIndex}>
                <InformationSect header={poiData[2].title} body={info[currentIndex].body} audioSrc={info[currentIndex].audio} />
            </PoiSlide>
            <SliderButtons currentIndex={currentIndex} setCurrentIndex={setCurrentIndex} info={info} />
            <PoiFooter hasGallery galleryLink={'/pointsofinterest/gardenOfHumanRights/gardenOfHumanRightsGallery'} />
        </>
    );
}